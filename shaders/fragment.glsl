...
void main()
{
    ...
    vec3 _pos = texture(def_position, norm_coord).xyz;
    ...
    vec3 light_vect = normalize(light_pos - _pos);
    vec3 view_vect = normalize(cam_pos - _pos);
    // Shadow cube
    float shadow = 1.0;
    float dist = length(_pos - light_pos);
    if (dist > texture(shadow_cube, -light_vect).r + 0.001)
        shadow = 0.5;
    ...
    float spec_coef = pow(max(dot(_normal, half_vect), 0), _shininess * 64);
